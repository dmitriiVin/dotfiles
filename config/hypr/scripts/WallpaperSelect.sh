...
wallDIR="$HOME/Pictures/wallpapers"  # ПРЯМОЙ ПУТЬ вместо $PICTURES_DIR
...
# ОТЛАДКА: Проверим, существует ли папка
echo "DEBUG: wallDIR = $wallDIR" > /tmp/wallpaper_debug.log
if [ -d "$wallDIR" ]; then
  echo "DEBUG: Папка существует" >> /tmp/wallpaper_debug.log
else
  echo "DEBUG: Папка НЕ существует!" >> /tmp/wallpaper_debug.log
  exit 1
fi

# Retrieve wallpapers (both images & videos) - УБРАЛ ФЛАГ -L
mapfile -d '' PICS < <(find "${wallDIR}" -type f \( \
  -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o \
  -iname "*.bmp" -o -iname "*.tiff" -o -iname "*.webp" -o \
  -iname "*.mp4" -o -iname "*.mkv" -o -iname "*.mov" -o -iname "*.webm" \) -print0)

# Отладка - какие файлы нашлись
echo "DEBUG: Найдено файлов: ${#PICS[@]}" >> /tmp/wallpaper_debug.log
echo "DEBUG: GIF файлы:" >> /tmp/wallpaper_debug.log
gif_count=0
for pic in "${PICS[@]}"; do
  if [[ "$pic" == *.gif ]]; then
    echo "  $pic" >> /tmp/wallpaper_debug.log
    ((gif_count++))
  fi
done
echo "DEBUG: Всего GIF: $gif_count" >> /tmp/wallpaper_debug.log

# Если гифок нет, попробуем найти их отдельно
if [ $gif_count -eq 0 ]; then
  echo "DEBUG: Ищем GIF отдельно:" >> /tmp/wallpaper_debug.log
  find "$wallDIR" -type f -iname "*.gif" -ls >> /tmp/wallpaper_debug.log 2>&1
fi
  ...
  for pic_path in "${sorted_options[@]}"; do
    ...
    if [[ "$pic_name" =~ \.gif$ ]]; then
      ...
      if [[ ! -f "$cache_gif_image" ]]; then
        ...
        echo "Создаю превью для $pic_name" >> /tmp/wallpaper_debug.log
        magick "$pic_path[0]" -resize 1920x1080 "$cache_gif_image" 2>> /tmp/wallpaper_debug.log
      fi
